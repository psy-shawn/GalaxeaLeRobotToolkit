...
def save_jsonl(data, file_path):
    ...
    """
    # Write JSONL ensuring non-ASCII characters (e.g., Chinese) are preserved
                ...
                for file in files:
                    if file.endswith(".parquet") and f"episode_{old_index:06d}" in file:
                        try:
                            ...
                            # 更新coarse_task_index和task_index列 (Update coarse_task_index and task_index column)
                            for column in ["task_index", "coarse_task_index", "quality_index", "coarse_quality_index"]:
                                if column in df.columns and old_folder in folder_task_mapping:
                                    # 获取当前task_index (Get current task_index)
                                    current_task_index = df[column].values

                                    # 检查是否有对应的新索引 (Check if there's a corresponding new index)
                                    if np.all(np.array([idx in folder_task_mapping[old_folder] for idx in current_task_index])):
                                        new_task_index = np.array([folder_task_mapping[old_folder][idx] for idx in current_task_index])
                                        # print(
                                        #     f"更新{column}从 {np.unique(current_task_index)} 到 {np.unique(new_task_index)}"
                                        # )
                                        df[column] = new_task_index
                                    else:
